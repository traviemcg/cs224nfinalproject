...
class AnsSoftmaxRegression(nn.Module):
    def __init__(self, seq_len, hidden_size):
        super(AnsSoftmaxRegression, self).__init__()
        self.seq_len = seq_len
        self.hidden_size = hidden_size
...
class MultiSoftmaxRegression():
    def __init__(self, seq_len, hidden_size):
        ...
        self.model_is_ans = AnsSoftmaxRegression(seq_len, hidden_size)
    ...
    def train_step(self, inputs, targets, mode, device):
        if mode == "start":
            ...
            model = self.model_is_ans.to(device)
        else:
            model = None
            assert model != None, "Invalid value mode={}, should be 'start', 'stop', or 'ans'!".format(mode)
    ...
    def predict(self, inputs, device):
        ...
        with torch.no_grad():
            ...
            is_ans = self.model_is_ans.to(device).eval().predict(inputs)
    ...
    def save(self, path):
        ...
        torch.save(self.model_is_ans.state_dict(), path+"_is_ans")
    ...
    def load(self, path):
        ...
        self.model_is_ans.load_state_dict(torch.load(path+"_is_ans"))
