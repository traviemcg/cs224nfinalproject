...
def eval_probe(probe_dir,
               ...
               test_file = "test-v2.0.json",
               device = "cpu"):

    # Extract examples
    tokenizer = AutoTokenizer.from_pretrained(model_prefix)
    processor = SquadV2Processor()
    examples = processor.get_dev_examples(data_dir = data_dir, filename = test_file)
    ...
    for i in range(layers):
        ...
        pred['Question'] = [""] * len(examples)
        pred['Score'] = [0] * len(examples)
